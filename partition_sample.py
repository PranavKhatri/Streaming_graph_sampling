...
1.generate 'incomplete' corresponding .comms file for every snapshot under ....
2.generate 'incomplete' corresponding .tcomms file for every snapshot with the same sample size under ./tcomms
'''
...
if __name__ == "__main__":
    ...
    snapshot_directory = ['old_pies_snapshot', 'new_pies_snapshot'];
    sample_size = ['0.15', '0.30', '0.45', '0.60']
    ...
    while ( i < total_directories ):
        ...
        while (j < total_samplesizes):
            ...
            while (k < total_snapshots):
                ...
                edgefile = './' + snapshot_directory[i] + '/' + sample_size[j] + '/output-prefix.t0000'+ str(k) + '.graph'
                ...
                for node in part.keys():
                    tcomms.append(str(k) + ' ' + str(node) + ' ' + str(part[node]) + '\n')
                    comms.append(str(node) + ' ' + str(part[node]) + '\n')
                
                #write the partition of snapshot t into separate file
                commsfile = './' + snapshot_directory[i] + '/' + sample_size[j] + '/output-prefix.t0000'+ str(k) + '.comms'
            ...
            tcommsfile = './tcomms/' + snapshot_directory[i] + '_' +  sample_size[j] + '_output-prefix.tcomms'
