...
#    parser.add_argument("-e", "--end", default=10, type=int, help="the file number at which to end, inclusive", dest="end")
 ...
 #   parser.add_argument("-o", "--output", default="generated_benches/", help="the output path, defaults to 'generated_benches/'", dest="output_directory_stem")
...
def runBigclam(input_file, number_of_clusters, outupt_prefix):
    ...
    process = subprocess.Popen(['./bigclam', '-i:', input_file, '-c:', str(number_of_clusters), '-o', outupt_prefix], cwd=bigclam_directory, stdout=subprocess.PIPE)
...
#    percentages = [0.15]
    for per in percentages:        
        ...
        results.append('Snapshot'+','+'JS_degree'+ ','+'KD_degree'+','+'JS_CC'+ ','+'KD_CC'+
            ','+'JS_path'+ ','+'KD_path'+','+'num_edges' + ','+'eccentricity'+ ','+'radius'+
            ','+'modularity'+','+'1.0-precision'+ ','+'1.0-recall'  + ','+'0.9-precision'+ ','+'0.9-recall' 
             + ','+'0.8-precision'+ ','+'0.8-recall'+ ','+'0.7-precision'+ ','+'0.7-recall'+ ','+'0.6-precision'+
              ','+'0.6-recall'+ ','+'0.5-precision'+ ','+'0.5-recall'+ ','+'mediate_size_micro'+ ','+ 'sample_cluster_num_micro'+ 
              ','+'original_cluster_num_micro'+ ','+'ANC'+ ','+'NLS' + ','+ 'NMI'+ ','+'ARS' +','+ '\n')
                ...
                # + ','+str(p8_precision_micro)+ ','+str(r8_recall_micro)+ ','+str(p7_precision_micro)+ ','+str(r7_recall_micro)+ ','+ str(p6_precision_micro)+ ','+
                # str(r6_recall_micro)+ ','+str(p5_precision_micro)+ ','+str(r5_recall_micro)+ ','+str(mediate_size_micro)+ ','+ str(sample_cluster_num_micro)+ ','+
                # str(original_cluster_num_micro)+ ','+str(ANC)+ ','+str(NLS)+ str(NMI)+ ','+str(ARS)+','+ '\n')
...
#            args.input_directory_stem='original_snapshot_20w/'
...
#==============================================================================
            ...
            results.append(str(i)+ ','+str(js_result_pies)+ ','+str(ks_result_pies)+ ','+str(cc_js_result_pies)+ ','+str(cc_ks_result_pies)+ ','
                ...
                +str(mediate_size_micro)+ ','+ str(sample_cluster_num_micro)+ ','+str(original_cluster_num_micro)+ ','+str(ANC)+ ','+str(NLS)+ ','+ str(NMI)+ ','+ str(ARS)+','+ '\n')
            print('results',results)  
            time.sleep(2)   
        we=open(data_set+'_'+sampling_algorithm+'_summary_Divergence_sample_p'+str(int(100*float(per))) + '.csv','wb')
        for line in results:
            we.write(line)
        we.close()
