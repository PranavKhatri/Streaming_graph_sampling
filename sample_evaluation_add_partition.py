...
#            print "Radius - "+str(nx.radius(original))
            if ground_truth==0:
                ...
                partition_set = se.ReadInData('original_snapshot/' + 'output-prefix.t0000'+ str(i) +'.comms') 
                partition_ground_truth = se.partitionFromFile('original_snapshot/' + 'output-prefix.t0000'+ str(i) +'.comms', ' ')
...
#            print round(per,2)
            per_format = ("%.2f" % per)
            full_partition_prefix = sampling_algorithm[0]+'_snapshot/'+str(per_format)+'/output-prefix.t0000'+ str(i) 
...
str(sample_cluster_num_micro)+ ','+str(original_cluster_num_micro)+ ','+str(ANC)+ ','+str(NLS) +','+ '\n')
        ...
        we=open(sampling_algorithm[0]+'_summary_Divergence_sample_p'+str(int(100*float(per))) + '.csv','wb')
        for line in results:
            we.write(line)
        we.close()
