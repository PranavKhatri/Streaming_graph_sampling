...
############summraize  the distribution
            ...
            if i== snapshot-1:
                ...
                # It means when the ./order_ start again, we should clean the folder. Only suitable for our case jianpeng
                if sampling_algorithm =='new_pies':
        ...
        average_result_single = list(pd.read_csv(Result_folder + data_set+'_'+clustering_algorithm+'_'+sampling_algorithm+'_summary_Divergence_sample_p_'+str(int(100*float(per))) + '.csv', sep=",", dtype="float64").mean())
        average_result_single.insert(0, sampling_algorithm)       
        newitem=[]
        for e in average_result_single:
            str_e = str(e)
        ...
        we=open(Result_folder +'All_'+data_set+'_'+clustering_algorithm+'_summary_Divergence_sample_p_'+str(int(100*float(per))) + '.csv','ab')
